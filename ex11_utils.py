...
def path_genarator_by_word(board: Board, path_list: list, board_height, board_width, path, length, words):
    if len(word_in_path(board, path)) == length:
        print(word_in_path(board, path))
        path_list.append(path)
        return
    for one_direction in valid_next_steps(path[-1], board_height, board_width):
        if one_direction not in path:
            path_genarator_by_word(board, path_list, board_height, board_width, path + [one_direction],length, words)
...
if __name__ == "__main__":
    ...
    # l = []
    # path_genarator_by_word(s, l, 4, 4, [(0,0)], ["CFY", "GAI"], 3)
    # print(l)